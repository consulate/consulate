...
app.defaultConfiguration = function(opts) {
  ...
  // API
  self
    .post(tokenPath, disallowCookie, verifyClient(opts.clients), oauth2.token(), errorLogger, oauth2.errorHandler());
  ...
  var storeAuthorizeParams = callbacks('storeAuthorizeParams', function(req, next) { next(); });

  function authorizeParamsMiddleware(req, res, next) {
    storeAuthorizeParams(req, next);
  }

  self
    .get(authorizePath, authorizeParamsMiddleware, login.ensureLoggedIn(), oauth2.authorizeClient(), oauth2.getPreviousDecision(), decision)
    .get(authorizePath, oauth2.authorizeLocals(), self.viewCallback('authorize'))
    .post(decisionPath, authorizeParamsMiddleware, login.ensureLoggedIn(), transactionLoader, oauth2.rememberDecision(), decision);
};
...
/**
 * Disallow setting cookies middleware
 */

function disallowCookie(req, res, next) {
...
app.callback = function(name, defaultCb) {
  ...
  // It's requesting the 'issueTokens' helper
  if (name === 'issueTokens') return issueTokens(this.callback.bind(this));
...
/**
 * Register a callback to verify a client provided secret
 *
 * @param {Function} fn
 * @api public
 */

app.verifySecret = function(fn) {
  debug('registering callback verifySecret');
  this.addCallback('verifySecret', fn);
  return this;
}
...
/**
 * Store any additional parameters from the request in the session
 *
 * @param {Function} fn
 * @api public
 */

app.storeAuthorizeParams = function(fn) {
  debug('registering callback storeAuthorizeParams');
  this.addCallback('storeAuthorizeParams', fn);
  return this;
};
...
app.authenticate = function(strategy, opts) {
  ...
  return function(req, res, next) {
    ...
    self._passport.authenticate(strategy, opts, function(err, user, info) {
      ...
      if (!user) {
        res.locals({loginError: 'invalid_user_credentials'});
        return next();
      }
...
/**
 * This is a computed callback for convenience of generating refresh and access tokens
 *
 * @api private
 */

function issueTokens(callbacks) {
  ...
  return function issueTokens(req, type, client, user, scope, done) {
    debug('issuing token for user', user.id);
    issueToken(req, client, user, scope, function(err, accessToken, params) {
      debug('issued token for user', user.id, accessToken);
      if (err) return done(err);
      ...
      createRefreshToken(req, client, user, scope, function(err, refreshToken, refreshTokenParams) {
        ...
        getAdditionalParams(req, type, client, user, scope, function(err, additionalParams) {
          if (err) return done(err);
