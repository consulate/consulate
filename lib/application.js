...
app.defaultConfiguration = function() {
  ...
  self.get('/authorize', login.ensureLoggedIn(), oauth2.authorize(), function(req, res, next) {
    // TODO populate the correct `res.locals` with the needed info
    res.locals({
      scopes: ['user:name'],
      clientName: 'Test Application',
      clientDescription: 'A testing client helping oauth implementors one app at a time'
    });
  }, self.viewCallback('authorize'));
};
