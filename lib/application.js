...
app.defaultConfiguration = function() {
  ...
  self.get("/approve", function(req, res, next) {
    // TODO populate the `locals` with the needed info
    res.locals({
      scopes: ["user:name"],
    ...
    // Call the registered callback
    self.viewCallbacks.approveClient(req, res, next);
  });
};
...
/**
 * Register a callback to render the 'approve-client' page
 *
 * @param {Function} fn
 * @api public
 */

app.approveClientView = function(fn) {
  this.viewCallbacks.approveClient = fn;
};
