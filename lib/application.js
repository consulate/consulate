...
app.defaultConfiguration = function(opts) {
  ...
  var paths = opts.paths || {}
    , tokenPath = paths.token || '/token'
    , loginPath = paths.login || '/login'
    , authorizePath = paths.authorize || '/authorize'
    , decisionPath = paths.decision || authorizePath + '/decision';
  ...
  // API
  self
    .post(tokenPath, client.verifyApp(), oauth2.token(), errorLogger, oauth2.errorHandler());
  ...
  self
    .get(authorizePath, login.ensureLoggedIn(), oauth2.authorizeClient(), oauth2.getPreviousDecision(), decision)
  ...
  return function() {
    ...
    (function pass(i) {
      ...
      args[args.length - 1] = function(err, obj) {
        ...
        if (obj || obj === null || obj === false || arguments.length < 2) return done(null, obj);
...
/**
 * Register a callback to filter the scopes for a request by the client
 *
 * @param {Function} fn
 * @api public
 */

app.filterScopesByClient = function(fn) {
  debug('registering callback filterScopesByClient');
  this.addCallback('filterScopesByClient', fn);
  return this;
};

/**
 * Register a callback to filter the scopes for a request by the user
 *
 * @param {Function} fn
 * @api public
 */

app.filterScopesByUser = function(fn) {
  debug('registering callback filterScopesByUser');
  this.addCallback('filterScopesByUser', fn);
  return this;
};

/**
 * Register a callback to return the default client scopes if none were passed with the request
 *
 * @param {Function} fn
 * @api public
 */

app.defaultClientScopes = function(fn) {
  debug('registering callback defaultClientScopes');
  this.addCallback('defaultClientScopes', fn);
  return this;
};
...
app.register = function(strategy) {
