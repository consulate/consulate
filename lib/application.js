...
app.defaultConfiguration = function() {
  ...
  var oauth2 = oauth2server(self.callback)
    ...
    , client = clientAuth(self.callback)
    , user = userAuth(self.callback);
  ...
  // API
  self
    .post('/token', client.verifyApp(), oauth2.token(), oauth2.errorHandler());
  ...
  self
    .get('/authorize', login.ensureLoggedIn(), oauth2.authorize(), oauth2.authorizeLocals(), self.viewCallback('authorize'))
    .post('/authorize', login.ensureLoggedIn(), oauth2.decision());
};
