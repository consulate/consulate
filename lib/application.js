...
app.defaultConfiguration = function() {
  ...
  var oauth2 = oauth2server(self.callback)
    , grants = registerGrants(oauth2, self.callback)
    , exchanges = registerExchanges(oauth2, self.callback)
    , client = clientAuth(self.callback('client'));
  ...
  // API
  self.post('/token', client.verifyApp(), oauth2.token(), oauth2.errorHandler());

  // UI
  self.get('/login', self.viewCallback('login')); // TODO populate `res.locals`
  self.post('/login', passport.authenticate('local', {
    successReturnToOrRedirect: '/',
    failureRedirect: '/login'
  }));

  self.get('/authorize', login.ensureLoggedIn(), oauth2.authorize(), oauth2.authorizeLocals(), self.viewCallback('authorize'));
  self.post('/authorize', login.ensureLoggedIn(), oauth2.decision());
};
...
app.viewCallback = function(name) {
  ...
  return function() {
    ...
    self.viewCallbacks[name].apply(null, arguments);
  };
};
