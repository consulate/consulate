...
app.defaultConfiguration = function(opts) {
  ...
  self
    .get(authorizePath, ensureLoggedIn(), oauth2.authorizeClient(), oauth2.authorizeLocals(), oauth2.getPreviousDecision(), decision)
    .get(authorizePath, self.viewCallback('authorize'))
    .post(decisionPath, ensureLoggedIn(), transactionLoader, oauth2.rememberDecision(), decision);

  function ensureLoggedIn() {
    return function (req, res, next) {
      var path = req.query.register
        ? registerPath
        : loginPath;

      login.ensureLoggedIn(path)(req, res, next);
    }
  }
};
  ...
  return function(req, res, next) {
    ...
    passport.authenticate(strategy, opts, function(err, user, info) {
      ...
      req.logIn(user, function(err) {
        ...
        var returnTo = req.session.returnTo || self.locals.defaultRedirectUrl;
        // Delete it
